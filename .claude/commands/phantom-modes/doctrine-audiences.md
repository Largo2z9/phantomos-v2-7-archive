...
══════════════════════════════════════════════
COMMENT CARTOGRAPHIER LES AUDIENCES · 4 questions
...
Q2 · NIVEAU DE GRANULARITÉ
   ...
   Pour descendre d'un niveau · faut 3/3 (volume + hook différent + offre différente)

Q3 · OÙ ELLE EN EST DANS SON PARCOURS
   2 axes modulateurs (pas 5 audiences mais 5 façons de lui parler) :
     ce qu'elle sait du produit         découvre → maitrise tout
...
NEXT SUGGESTED
  → Tape : `/phantom {brand} audiences` (vue brand-side)
  → Tape : `cartographier une nouvelle audience` (lance le skill)
  → Tape : `voir le cours complet` (doctrine pédago étendue)
```
