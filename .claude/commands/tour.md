...
| *"creuse tous les pain points de la brand"* | `/phantom {brand} pain-points` cross-audiences view (v2.63 ontologie pure) |
| *"liste les objections bloquantes"* | `/phantom {brand} objections --severity=blocking` (v2.63 cross-audiences PNT-NN/OBJ-NN canonical) |
...
- **Pipeline DTC paid acquisition (textuel)** · comment `/build-atlas-complete {brand}` chain 9 sub-skills auto from blank URL (setup → snapshot → deepen-brand-context → profile-audience → weight-dimensions → produce-paid-angles → score-matrix → produce-copy-brief → compose-creative) avec gates opérateur entre phases, OR séquentiel manuel snapshot → mine-voc → produce-paid-angles → produce-copy-brief si l'opérateur préfère contrôle drill par drill. **v2.63 · cartographie audience canonical** · `profile-audience` ne stocke plus les pain_points + objections inline dans `audience.profile.json` (legacy v2.60). Les collections sub-tensions sont désormais séparées top-level (`brands/{brand}/pain_points/PNT-NN.json` + `brands/{brand}/objections/OBJ-NN.json` + `brands/{brand}/frictions/FRC-NN.json`), 3 collections orthogonales cross-audiences. Visibility cross-audience d'une même PNT-NN ou OBJ-NN encodée 1 fois canonical, drill `/phantom {brand} pain-points` et `/phantom {brand} objections` cross-audiences. Pertinent quand l'opérateur a signalé un profil paid manager / agency / DTC media buyer en Milestone 1, 2 ou 4.
