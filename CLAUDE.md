...
`brands/_EXAMPLE/` contient un cas pédagogique partagé canon (Stepprs · brand pédagogique cas canonique · cf `brands/_EXAMPLE/README.md` pour contexte complet). Cas illustratif pour rendre tangibles les concepts canon PhantomOS, JAMAIS la marque active de l'opérateur.
...
- **Decomposition Visibility Discipline** (v2.79+ · v2.81.1+ NEW NIVEAU LIVE thinking aloud), `docs/system/decomposition-visibility-discipline.md` (l'agent doit PROUVER qu'il a compris en MONTRANT sa réflexion · 4 niveaux matriciels obligatoires + pédagogie verbale cross-skills opérateur-facing · 3 phases temporelles canon · avant NIVEAU 0 paramètres pré-exec · pendant NIVEAU LIVE thinking aloud expert · après NIVEAUX 1-4 matrices post-exec).
- **Output Clarity Discipline** (v2.79.2+), `docs/system/output-clarity-discipline.md` (standards Vercel / GitHub-grade shipped product cross-outputs opérateur-facing · 6 standards canon iconographie unique + dejargonisation + headers FR sobres + one thing per line + action items isolés + density modérée · 8 Hard Rules enforcement runtime).
- **Onboarding Holistic Discipline** (v2.79.3), `docs/system/onboarding-holistic-discipline.md` (onboarding agnostique + holistique · panorama 360° toutes capacités · zéro typage opérateur entrée · l'opérateur se reconnaît dans le panorama).
- **Entry Arc Discipline** (v2.81.0), `docs/system/entry-arc-discipline.md` (post-tour multi-entry 4 portes MECE · A conversationnel · B brand-first · C import existant · D progressif libre · first 30 minutes canon + canons Vincent runtime + ton premium enforcement).
- **Engagement Disclosure Discipline** (v2.79.3), `docs/system/engagement-disclosure-discipline.md` (skills orchestrateurs lourds annoncent plan + ETA + démarche + confirmation pré-engagement · opérateur s'engage en conscience).
- **Update Distribution Discipline** (v2.80.0), `docs/system/update-distribution-discipline.md` (pattern de mise à jour PhantomOS opérateur-facing · preserve operator state · migrations versionnées · semver strict · backup + rollback canon).
...
- "/bird {brand}" / "/breakdown {brand}" commands (pédagogie reverse engineering)
...
| **Onboarding holistique agnostique.** /tour expose panorama 360° des 7 territoires métiers DTC (Creative · Tracking · Media Buy · Brand · Ops · Business · Lifecycle) sur pied d'égalité. L'opérateur découvre l'étendue, se reconnaît dans le panorama, choisit où commencer. Pas de typage initial. Full doctrine `docs/system/onboarding-holistic-discipline.md`. | **NEVER** demander "tu fais quoi", "ton métier", "ton rôle", "ton profil" en porte d'entrée. **NEVER** adapter la surface pré-emptive selon profil métier déclaré. **NEVER** enum métier figé dans `operator/profile.json`. |
| **Pre-engagement disclosure obligatoire.** Avant tout skill `type: orchestrator` OU duration estimée >5 min OU spans sessions OU 2+ sub-skills déployés OU producer heavy paid (build-atlas-complete · onboard-brand · mine-voc · setup-brand · creative-brief-composer · produce-paid-matrix · produce-paid-angles · produce-copy-brief · compose-creative · audit-meta-account · launch-intelligence-deep), l'agent expose disclosure pré-engagement · plan (N étapes ≤8) + ETA chiffrée (minutes ou range) + démarche (implication + livrable) + close binaire confirmation. Full protocol `docs/system/engagement-disclosure-discipline.md`. | **NEVER** lancer skill orchestrator silent sans disclosure. **NEVER** ETA vague ("rapide", "long", "patiente"). **NEVER** court-circuit silent (skip disclosure sans opt-out explicit operator). **NEVER** opt-in default · canon est opt-out expert post-N usages. |
| **Update workspace canon.** `/update` permet à l'opérateur de mettre à jour le workspace vers la dernière version canon · preserve strict ses brands + operator state + credentials + session-state · migrations versionnées automatiques pour BREAKING changes · backup pre-migration obligatoire `_archive/migrations/pre-v{version}-{date}/` · rollback path canon via `/update --rollback {version}`. Disclosure pré-update obligatoire (cohérent engagement-disclosure-discipline v2.79.5 + NIVEAU 0 paramètres décomposés). Full doctrine `docs/system/update-distribution-discipline.md`. | **NEVER** update silently sans disclosure. **NEVER** écraser brands/ ou operator/ ou credentials. **NEVER** apply BREAKING sans migration script idempotent. **NEVER** absence de backup pre-migration. **NEVER** absence de rollback path. |
